...
import { Character, DialogueLine, GoogleUser } from '../types';
...
const StoryPlayerModal: React.FC<StoryPlayerModalProps> = ({ story, party, user, onClose }) => {
  ...
  }, [lineIndex, story, typedText, currentLine, onClose, processText]);
  ...
  const speakerInfo = charactersMap.get(currentLine.speaker);

  const getCharacterPosition = (speakerId: string) => {
    const speakersInScene = [...new Set(story.map(s => s.speaker))].filter(id => id !== 'commander');
    const speakerIndex = speakersInScene.indexOf(speakerId);
    if (speakerIndex === -1) return 'translateX(0)';
    const totalSpeakers = speakersInScene.length;
    const position = (speakerIndex - (totalSpeakers - 1) / 2) * 35;
    return `translateX(${position}%)`;
  };

  return (
    <div className="absolute inset-0 z-50 h-full w-full bg-black/80 backdrop-blur-md flex flex-col p-6 text-white cursor-pointer animate-fadeIn" onClick={handleNext}>
      <div className="relative z-10 flex-grow w-full flex flex-col items-center justify-center">
        {party.map(p => {
          const isSpeaking = currentLine.speaker === p.id;
          const isInScene = story.some(s => s.speaker === p.id);
          ...
          return (
            <img 
              ...
              src={p.avatar} 
              alt={p.name}
              className={`absolute bottom-0 h-[80%] object-contain transition-all duration-500 ${isSpeaking ? 'brightness-110 scale-105' : 'brightness-75'}`}
              style={{ transform: getCharacterPosition(p.id) }}
            />
          )
        })}
        <img
          ...
          alt="단장"
          className={`absolute bottom-0 h-[85%] object-contain transition-opacity duration-500 ${currentLine.speaker === 'commander' ? 'opacity-100' : 'opacity-0'}`}
        />
      </div>
